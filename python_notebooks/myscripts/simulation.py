...
def c1(env, cust_num, counter, service, cust_dict, verbose):
    ...
    with counter.request() as req:   # 没被占用可以向柜台发送请求，通过开始进行req:
        yield req
...
def s2(env, interval, counter, service, cust_dict, verbose=False):
    ...
    while True:
        i += 1
        inter_arr = rng.exponential(interval) + 0.1
        yield env.timeout(inter_arr)
...
def c2(env, cust_num, counter, service, cust_dict, verbose):
    ...
    with counter.request() as req:
        patience = rng.uniform(0.5, 4.0)  # 新参数1:顾客的等待时间耐心服从均匀分布
        amt_spent = rng.gamma(3, 2)		  # 新参数2:顾客平均的花费服从gamma分布，是指数分布的泛化
